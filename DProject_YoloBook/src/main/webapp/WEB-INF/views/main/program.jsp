	    ...
	    <section class="portfolio" id="portfolio">
	      <div class="container">
			...
			<c:forEach var="programlist" items="${programlist }" varStatus="status">
				<c:if test="${programlist.p_status eq '3'}">
			          <div class="col-md-6 col-lg-4">
			            ...
			            <a class="portfolio-item d-block mx-auto" href="#portfolio-modal-${status.count}" id="test"> <!-- "#portfolio-modal-${status.count} 를 통해 이미지 누를때 modal을 뿌려줄 수 있다. -->
			              ...
			              <c:if test="${programlist.p_class eq '1'}">
			              	<img class="img-fluid" src="resources/main_bootstrap/img/portfolio/cake/${programlist.p_thumbnail_filename}" alt="">
			              </c:if>
			              <c:if test="${programlist.p_class eq '2'}">
			              	<img class="img-fluid" src="resources/main_bootstrap/img/portfolio/jasu/${programlist.p_thumbnail_filename}" alt="">
			              </c:if>	
					      <c:if test="${programlist.p_class eq '3'}">
			              	<img class="img-fluid" src="resources/main_bootstrap/img/portfolio/leather/${programlist.p_thumbnail_filename}" alt="">
			              </c:if>	
					      <c:if test="${programlist.p_class eq '4'}">
			              	<img class="img-fluid" src="resources/main_bootstrap/img/portfolio/art/${programlist.p_thumbnail_filename}" alt="">
			              </c:if>	
					       ${programlist.p_name }
			            </a>
			          </div>
...
<c:forEach var="i" items="${programlist }" varStatus="status"> <!-- db의 class가 1일때를 조건으로 -->
        ...
        <div class="container text-center">
          <div class="row">
            <div class="col-lg-8 mx-auto">
              <h2 class="text-secondary text-uppercase mb-0">${i.p_name }</h2>
              <hr class="star-dark mb-5">
              
              <c:if test="${i.p_class eq '1' }"> 
             	 <img class="big-image" src="resources/main_bootstrap/img/portfolio/cake/${i.p_thumbnail_filename}" alt="">
              </c:if>
              <c:if test="${i.p_class eq '2' }"> 
             	 <img class="big-image" src="resources/main_bootstrap/img/portfolio/jasu/${i.p_thumbnail_filename}" alt="">
              </c:if>
              <c:if test="${i.p_class eq '3' }"> 
             	 <img class="big-image" src="resources/main_bootstrap/img/portfolio/leather/${i.p_thumbnail_filename}" alt="">
              </c:if>
              <c:if test="${i.p_class eq '4' }"> 
             	 <img class="big-image" src="resources/main_bootstrap/img/portfolio/art/${i.p_thumbnail_filename}" alt="">
              </c:if>
              
              <p class="mb-5"> ${i.p_introduction }</p>
              <a class="nav-link py-3 px-0 px-lg-3 rounded js-scroll-trigger" href="book?p_number=${i.p_number}">예약하기</a>
			  <!--<a class="btn btn-primary btn-lg rounded-pill portfolio-modal-dismiss" href="order"> -->
            </div>
          </div>
        </div>
      </div>
