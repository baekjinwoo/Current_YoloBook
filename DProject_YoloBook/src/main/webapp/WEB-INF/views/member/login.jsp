...
<div class="wrapper">
	<div class="container">
		...
		<form action="login" method="post" class="form" onsubmit="infoCheck(); return false;">
...
<!-- 아이디는 맞고, 암호가 틀릴경우 -->
<c:if test="${!empty loginResultMsg }"> 
	<script> alert("${loginResultMsg}")</script>
</c:if>
...
<script src="resources/jquery-3.2.1.min.js"></script>
	...
	function infoCheck() {
		$.ajax({
			async:true,
			url : "/program/idcheck", //web-program-settimg명 / mapping url명
			type : "POST",
			data : {"id":$("#id").val()},
			success : function(idcheck) {
				if (!idcheck) {
					alert("회원가입후 이용바랍니다."); //아이디가 null값일 경우
					document.getElementById("id").value = "";
					document.getElementById("pw").value = "";
					location.href="register"; //회원가입창으로 강제이동
				}
				else {
